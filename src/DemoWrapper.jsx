...
const ICONS = { "Class Scheduling": CalendarCheck, "Practice Tracking": Activity, "Community Feed": Heart, "Teacher Profiles": Users, "Membership Tiers": CreditCard, "Events & Workshops": Sparkles, "Smart Notifications": Bell, "Admin Dashboard": Shield, "All-in-One Platform": MapPin, "Member Engagement": Flame, "Custom-Built for You": Palette };
...
export default function DemoWrapper() {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#f5f3f0", fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      {/* LEFT SIDEBAR */}
      <aside style={{ width: 320, flexShrink: 0, position: "fixed", top: 0, left: 0, bottom: 0, overflowY: "auto", padding: "32px 28px", borderRight: "1px solid #e5e0d8", scrollbarWidth: "none" }}>
      ...
      <div style={{ flex: 1, display: "flex", justifyContent: "center", alignItems: "flex-start", marginLeft: 320, marginRight: 340, paddingTop: 24, paddingBottom: 24 }}>
        <div style={{ width: 390, minHeight: "100vh", position: "sticky", top: 24, background: "#fff", borderRadius: 20, boxShadow: "0 8px 40px rgba(0,0,0,.12), 0 1px 3px rgba(0,0,0,.08)", overflow: "hidden", transform: "translateZ(0)" }}>
          <div style={{ height: "calc(100vh - 48px)", overflow: "auto", scrollbarWidth: "none" }}>
            <style>{`::-webkit-scrollbar { display: none; }`}</style>
            <App />
          </div>
        </div>
      ...
      {/* RIGHT SIDEBAR */}
      <aside style={{ width: 340, flexShrink: 0, position: "fixed", top: 0, right: 0, bottom: 0, overflowY: "auto", padding: "32px 28px", borderLeft: "1px solid #e5e0d8", scrollbarWidth: "none" }}>
        <style>{`::-webkit-scrollbar { display: none; }`}</style>
